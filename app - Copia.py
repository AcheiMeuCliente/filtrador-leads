...
/* ── "↺ Limpar" — ghost/link button inline no header ── */
div[class*="st-key-reset_all_btn"] button{
  background:transparent !important; border:none !important; box-shadow:none !important;
  color:#6366f1 !important; font-size:12px !important; font-weight:700 !important;
  padding:4px 2px !important; justify-content:flex-end !important; width:auto !important;
}
div[class*="st-key-reset_all_btn"] button:hover{ color:#4338ca !important; text-decoration:underline; }

/* ── "Aplicar" nas Visualizações Rápidas — vira link, não botão ── */
div[class*="st-key-ps_btn_"] button{
  background:transparent !important; border:none !important; box-shadow:none !important;
  color:#4f46e5 !important; font-weight:700 !important; font-size:12.5px !important;
  padding:4px 2px !important; justify-content:flex-end !important;
}
div[class*="st-key-ps_btn_"] button:hover{ color:#4338ca !important; text-decoration:underline; background:transparent !important; }

/* ── Segmentos de Beleza — linhas de toggle (lista, não multiselect) ── */
div[class*="st-key-seg_row_"]{ margin-bottom:6px !important; }
div[class*="st-key-seg_row_"] button{
  justify-content:flex-start !important; text-align:left !important;
  padding:9px 12px !important; border-radius:9px !important; font-size:12.5px !important;
}
div[class*="st-key-seg_row_"] button p{ text-align:left !important; }

/* ── Estados (UF) — pílulas compactas em grid ── */
div[class*="st-key-uf_btn_"]{ margin-bottom:6px !important; }
div[class*="st-key-uf_btn_"] button{
  padding:6px 4px !important; border-radius:8px !important; font-size:12px !important; font-weight:700 !important;
}

/* Botões "primary" usados como estado selecionado nos toggles do menu */
...
.fav-star-btn{
  width:30px; height:30px; border-radius:8px; flex-shrink:0; align-self:flex-start;
  display:flex; align-items:center; justify-content:center; text-decoration:none !important;
  background:#fff; border:1px solid #e2e8f0; color:#cbd5e1 !important; font-size:16px; line-height:1;
  transition:all .12s ease;
}
.fav-star-btn:hover{ border-color:#fbbf24; color:#f59e0b !important; background:#fffbeb; }
.fav-star-btn.is-fav{ background:#fffbeb; border-color:#fbbf24; color:#f59e0b !important; }
...
.c-email-text{ font-family:var(--mono); font-size:12px; color:#334155; text-transform:lowercase; word-break:break-all; }
.c-right-group{ display:flex; align-items:center; gap:7px; flex-wrap:wrap; }
.c-icon-square-btn{
  width:30px; height:30px; border-radius:8px; border:1px solid #e2e8f0; background:#ffffff;
  color:#64748b !important; display:inline-flex; align-items:center; justify-content:center;
  text-decoration:none !important; cursor:pointer; transition:all .12s ease;
}
.c-icon-square-btn:hover{ background:#f8fafc; border-color:#cbd5e1; color:#0f172a !important; }
.c-btn-ext-link{ background:#dcfce7; border:1px solid #86efac; color:#15803d !important; }
.c-btn-ext-link:hover{ background:#bbf7d0; color:#166534 !important; }
.c-btn-abordar{
...
.act-btn-new{
  ...
  color:#334155 !important; font-size:12px; font-weight:600; text-decoration:none !important;
  display:inline-flex; align-items:center; justify-content:center; gap:6px; transition:all .12s ease;
}
.act-btn-new:hover{ background:#f8fafc; border-color:#cbd5e1; color:#0f172a !important; }
...
/* Alvos de toque maiores em botões da sidebar (acessibilidade mobile) */
[data-testid="stSidebar"] .stButton > button{ min-height:38px; }
...
/* ── Tablet e telas menores (≤900px): aperta respiro geral ── */
@media (max-width: 900px){
  .block-container{ padding-top:1rem; padding-left:0.9rem !important; padding-right:0.9rem !important; }
  .kpi{ padding:12px 14px; }
  .kpi-value{ font-size:22px; margin-top:6px; }
  .kpi-label{ font-size:10px; }
  .kpi-sub{ font-size:11px; }
  .stTabs [data-baseweb="tab-list"]{ overflow-x:auto; flex-wrap:nowrap; -webkit-overflow-scrolling:touch; scrollbar-width:thin; }
  .stTabs [data-baseweb="tab"]{ height:36px; padding:0 12px; font-size:12.5px; white-space:nowrap; flex-shrink:0; }
  .action-buttons-new{ grid-template-columns:repeat(2, 1fr); }
}

/* ── Celular (≤560px): correções críticas de overflow ── */
@media (max-width: 560px){
  /* Zoom automático do iOS em inputs com fonte <16px */
  [data-testid="stSidebar"] [data-baseweb="input"] input,
  [data-testid="stSidebar"] [data-baseweb="select"] div,
  ...
  .plan-chip{ font-size:9.5px; padding:3px 8px; margin-left:6px; }

  /* Card: cabeçalho quebra em duas linhas em vez de estourar.
     Linha 1 = avatar+nome+estrela (compartilham espaço); linha 2 = painel de
     metadados forçado a 100% de largura via flex-basis — não depende do
     conteúdo caber, é garantido sempre quebrar sozinho. */
  .card-top{ padding:13px 13px 9px; }
  .card-header-flex{ flex-wrap:wrap; gap:8px 10px; }
  .card-identity{ flex:1 1 auto; min-width:0; }
  .fav-star-btn{ flex-shrink:0; }
  .card-meta-panel{
  ...
  /* Linha de contato: empilha label e ações em vez de espremer */
  .contact-pill-box{ padding:10px 13px; }
  .c-row-new{ flex-direction:column; align-items:flex-start; gap:7px; }
  .c-right-group{ width:100%; flex-wrap:wrap; justify-content:flex-start; }

  .action-buttons-new{ grid-template-columns:repeat(2, 1fr); padding:10px 13px; gap:7px; }
  .act-btn-new{ font-size:11.5px; padding:8px 6px; }

  details.card-expand summary{ padding:10px 13px; }
  .expand-body{ padding:12px 13px; }

  /* KPIs em coluna única legível */
...
def format_phone_display(phone_str):
    ...
    # Não é um telefone BR válido (ex: link de wa.me ou artefato numérico salvo na
    # coluna errada) — melhor esconder do que exibir lixo pro usuário.
    return ""
...
def clean_field(v):
    """None/NaN/'nan' viram string vazia — evita vazar lixo pro dossiê de busca."""
    if v is None:
        return ""
...
def build_smart_search_url(row):
    """Link de Busca com IA do Google (udm=50) usando Entity Stacking + Google
    Dorks — a técnica de OSINT pra esse tipo de busca: nada de "roleplay" de
    pesquisador, só as âncoras empilhadas puras, com aspas nas que precisam
    de correspondência exata (CNPJ/telefone/e-mail são "impressões digitais":
    sem aspas o Google devolve resultado aproximado, não a página exata onde
    o lojista digitou aquele dado)."""
    fantasia = clean_field(row.get("NOME FANTASIA"))
    razao = clean_field(row.get("RAZÃO SOCIAL"))
    nome = fantasia or razao
    if not nome:
        return ""

    # Âncora rígida 1: CNPJ — identificador único, o mais forte de todos pra
    # cruzar com diretórios/cadastros (Reclame Aqui, Google Meu Negócio etc.)
    cnpj = clean_phone(clean_field(row.get("CNPJ")))
    str_cnpj = f'"{cnpj}"' if cnpj else ""

    # Âncora rígida 2: telefone/WhatsApp — WHATSAPP_1 primeiro, TELEFONE_1 como
    # fallback. Usa string já limpa (não `a or b` cru: valor NaN de coluna
    # numérica é "truthy" em Python e quebraria o fallback silenciosamente).
    telefone = clean_phone(clean_field(row.get("WHATSAPP_1")))
    if not telefone:
        telefone = clean_phone(clean_field(row.get("TELEFONE_1")))
    str_telefone = f'"{telefone}"' if telefone else ""

    # Âncora rígida 3: e-mail — nunca o de contabilidade (puxaria a IA pro
    # escritório contábil, não pro negócio em si).
    email = "" if row.get("EMAIL_CONTABILIDADE") else clean_field(row.get("E-MAIL"))
    str_email = f'"{email}"' if email else ""

    # Triangulação geográfica (geofencing textual): bairro+município+UF+CEP
    # empilhados ajudam o Google a cruzar o nome com a base do Maps.
    bairro = clean_field(row.get("BAIRRO"))
    municipio = clean_field(row.get("MUNICIPIO"))
    estado = clean_field(row.get("ESTADO"))
    cep = clean_phone(clean_field(row.get("CEP")))

    # Direcionamento de superfície: nomeia as plataformas-alvo em vez de pedir
    # "redes sociais" genericamente, priorizando-as sobre agregadores de CNPJ.
    plataformas = "instagram facebook linkedin site google maps whatsapp"

    elementos = [nome, str_cnpj, bairro, municipio, estado, cep, str_email, str_telefone, plataformas]
    query = " ".join(e for e in elementos if e)
    query = " ".join(query.split())

    # Micro-comando estrutural: fecha com um pedido de formatação, não de
    # personagem — é o que faz o Modo IA devolver lista limpa em vez de bloco
    # de texto corrido.
    prompt = f"{query} listar links oficiais encontrados e resumo da atividade"
    return f"https://www.google.com/search?q={quote_plus(prompt)}&udm=50&hl=pt-BR&gl=br"
...
def active_filter_chips(filters):
    ...
    if filters.get("apenas_favoritos"):
        chips.append(("★ Apenas Favoritos", "apenas_favoritos", None))
...
def show_sidebar(df):
    ...
    with st.sidebar:
        ...
        # Header de Filtros com contador e botão de limpar (mesma linha)
        total_active = len(active_filter_chips(prev))
        h_col1, h_col2 = st.columns([2.4, 1])
        with h_col1:
            st.markdown(
                f"""
                <div class="sidebar-title" style="padding-top:6px;">
        ...
        with h_col2:
            if total_active > 0:
                if st.button("↺ Limpar", key="reset_all_btn", use_container_width=True):
                    reset_filters()
                    st.rerun()
        st.markdown('<hr style="margin:6px 0 12px !important;">', unsafe_allow_html=True)
        ...
        for i, ps in enumerate(preset_list):
            ...
            with c_p1:
                st.markdown(f'<div style="font-size:12px;font-weight:600;color:#334155;padding-top:9px;">{ps["name"]}</div>', unsafe_allow_html=True)
            with c_p2:
        ...
        with st.expander(group_title("📞 Canais de Contato", ["tem_email", "sem_contador", "tem_whatsapp", "apenas_favoritos"], prev), expanded=True):
            ...
            st.checkbox("⭐ Apenas Favoritos", key=k("apenas_favoritos"))

        with st.expander(group_title("🛍️ Segmentos de Beleza", ["segmentos", "origem_cnae"], prev), expanded=False):
            sel_segs = set(st.session_state.get(k("segmentos"), []))
            for name, cfg in SEG_CFG.items():
                is_sel = name in sel_segs
                label = f"{cfg['e']}  {name}" + ("   ✓" if is_sel else "")
                if st.button(label, key=f"seg_row_{name}", use_container_width=True,
                             type="primary" if is_sel else "secondary"):
                    if is_sel:
                        sel_segs.discard(name)
                    else:
                        sel_segs.add(name)
                    st.session_state[k("segmentos")] = list(sel_segs)
                    st.rerun()
            st.selectbox(
                "Origem do CNAE:",
        ...
        with st.expander(group_title("📍 Localização", ["estados", "municipios", "bairros"], prev), expanded=False):
            ...
            st.markdown('<div style="font-size:12px;font-weight:600;color:#475569;margin-bottom:6px;">Estados (UF)</div>', unsafe_allow_html=True)
            sel_uf = set(st.session_state.get(k("estados"), []))
            uf_cols = st.columns(4)
            for i, uf in enumerate(estados):
                is_sel = uf in sel_uf
                with uf_cols[i % 4]:
                    if st.button(uf, key=f"uf_btn_{uf}", use_container_width=True,
                                 type="primary" if is_sel else "secondary"):
                        if is_sel:
                            sel_uf.discard(uf)
                        else:
                            sel_uf.add(uf)
                        st.session_state[k("estados")] = list(sel_uf)
                        st.rerun()

            sel_uf_list = st.session_state.get(k("estados"), [])
            base = df[df["ESTADO"].isin(sel_uf_list)] if sel_uf_list else df
            ...
            st.multiselect("Municípios", mun_opts, key=k("municipios"), placeholder="Todos")
...
def apply_filters(df, filters):
    ...
    if filters.get("apenas_favoritos"):
        ...
        mask &= df["CNPJ"].astype(str).isin(favoritos)
...
# ══════════════════════════════════════════════════════════════
def build_card_html(row, is_fav=False):
    ...
    cnpj_raw = escape(str(row.get("CNPJ", "")).strip(), quote=True)
    fav_btn_html = (
        f'<a class="fav-star-btn{" is-fav" if is_fav else ""}" href="?fav_toggle={quote(cnpj_raw)}" '
        f'title="{"Remover dos favoritos" if is_fav else "Marcar como favorito"}">{"★" if is_fav else "☆"}</a>'
    )
    ...
    smart_search_url = escape(build_smart_search_url(row), quote=True)
    ...
    web_btn = (
        f'<a class="act-btn-new" href="{smart_search_url}" target="_blank" '
        f'title="Busca com IA do Google: redes sociais, Maps, site e WhatsApp">{SVG_GOOGLE_GLOBE} Busca IA</a>'
        if smart_search_url else
        f'<span class="act-btn-new" style="opacity:.4">{SVG_GOOGLE_GLOBE} Sem dados</span>'
    )
...
def main():
    ...
    if "fav_toggle" in st.query_params:
        cnpj_toggle = st.query_params["fav_toggle"]
        favoritos = st.session_state.setdefault("favoritos", set())
        if cnpj_toggle in favoritos:
            favoritos.discard(cnpj_toggle)
        else:
            favoritos.add(cnpj_toggle)
    ...
    with tab_cards:
        if total_leads == 0:
            ...
            for i, (_, row) in enumerate(df_page.iterrows()):
                with cols_cards[i % 2]:
                    is_fav = str(row.get("CNPJ", "")).strip() in favoritos
                    st.markdown(minify(build_card_html(row, is_fav)), unsafe_allow_html=True)
        ...
        if total_leads > 0:
            ...
            for _, row in df.iterrows():
                ...
                disp_data.append({
                    ...
                    "Busca IA": build_smart_search_url(row),
                })
            st.dataframe(
                ...
                hide_index=True,
                column_config={
                    "Busca IA": st.column_config.LinkColumn(
                        "Busca IA",
                        help="Abrir dossiê OSINT da empresa no Modo IA do Google",
                        display_text="🔍",
                        width="small",
                    )
                },
