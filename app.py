...
def show_list(df):
    ...
    for _, row in df_page.iterrows():
        ...
        wa_val = next((str(row.get(f"WHATSAPP_{i}","")) for i in range(1,4) if str(row.get(f"WHATSAPP_{i}","")).strip() and str(row.get(f"WHATSAPP_{i}","")).strip() != "nan"), "")
        tel_val = next((str(row.get(f"TELEFONE_{i}","")) for i in range(1,4) if str(row.get(f"TELEFONE_{i}","")).strip() and str(row.get(f"TELEFONE_{i}","")).strip() != "nan"), "")
        ...
        abertura = pd.to_datetime(row.get("INICIO ATIVIDADE","")).strftime("%d/%m/%Y") if pd.notna(row.get("INICIO ATIVIDADE")) else "—"
